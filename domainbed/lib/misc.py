...
def accuracy_gen(network, loader, weights, device,noise_sd=0.5,addnoise=False):
    correct = 0
    total = 0
    weights_offset = 0
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            x_gs = []
            for i in range(5):
                x_gs.append(batch["x_g%d" % i].to(device))
            # for i in range(4):
            #     x_gs.append(batch["x_g%d" % i].to(device))
            # y_g = y_g.to(device)
            if(addnoise):
                x=x + torch.randn_like(x, device='cuda') * noise_sd
            p = network.predict(x, x_gs)
            if weights is None:
                batch_weights = torch.ones(len(x))
            else:
                batch_weights = weights[weights_offset: weights_offset + len(x)]
...
def accuracy_clip(network, loader, weights, device,noise_sd=0.5,addnoise=False):
    correct = 0
    total = 0
    weights_offset = 0
...
def accuracy_causal(network, loader, random_loader, weights, device,noise_sd=0.5,addnoise=False):
    correct = 0
    total = 0
    weights_offset = 0
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            styles_device = [(x.to(device), y.to(device))
                              for x, y in next(random_loader)]
            # y_g = y_g.to(device)
            if(addnoise):
                x=x + torch.randn_like(x, device='cuda') * noise_sd
            p = network.predict(x, styles_device)
            if weights is None:
                batch_weights = torch.ones(len(x))
            else:
                batch_weights = weights[weights_offset: weights_offset + len(x)]
