...
class DecTransformer(nn.Module):
    def __init__(self, embed_dim, depth=12, num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, mask=None):
        ...
        if mask is not None:
            x = x[mask==0].reshape(B, -1, x.shape[-1])
        ...
        # out = layer_wise_tokens[-1]
        return layer_wise_tokens[-1]

    def forward(self, x,return_feat=False, mask=None, only_feat=False):
        out = self.forward_features(x, mask)
        features = out
