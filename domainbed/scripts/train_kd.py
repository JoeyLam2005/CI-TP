...
if __name__ == "__main__":
    ...
    parser.add_argument('--prompt_dir', type=str, default="./domainbed/prompt_path")
    ...
    random_loaders = [InfiniteDataLoader(
        dataset=env,
        weights=env_weights,
        batch_size=hparams['batch_size'],
        num_workers=dataset.N_WORKERS)
    ...
    # eval_datasets = torch.utils.data.ConcatDataset([env for env, _ in (in_splits + out_splits + uda_splits)])
    # eval_loaders = FastDataLoader(
    ...
    random_minibatches_iterator = zip(*random_loaders)
    ...
    for step in range(start_step, n_steps):
        ...
        minibatches_device = [batch[0].to(device), batch[1].to(device)]
        styles_device = [(x.to(device), y.to(device))
                              for x, y in next(random_minibatches_iterator)]
        if args.task == "domain_adaptation":
            uda_device = [(x.to(device), x_g.to(device))
                          for x, _, x_g, _ in next(uda_minibatches_iterator)]
        else:
            uda_device = None
        step_vals = algorithm.update(minibatches_device, styles_device, uda_device)
        ...
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
            ...
            for name, loader, weights in evals:
                acc = misc.accuracy_causal(algorithm, loader, random_minibatches_iterator, weights, device)
